...
async function _clusterList(articles: any[]) {
    ...
    for (const art of articles) {
        // create article tokens using ONLY title and summary (NOT content) to fix asymmetric Jaccard score
        const artText = `${art.title || ""} ${art.summary || ""}`;
        ...
        const s = (art.sentiment || "neutral") as "positive" | "neutral" | "negative";
